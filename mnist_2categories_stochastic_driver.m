function mnist_2categories_stochastic_driver()
close all
fsz = 20;
...
d = nPCA;
w = ones(d^2+d+1,1);
%% optimize w and b using a smooth loss function and stocastic optimizers
...
tol = 1e-3;
%
[w,f,gnorm] = stochasticGradientDescent(bsz,fun,gfun,w,kmax,tol);
%[w,f,gnorm] = stochasticAdam(bsz,fun,gfun,w,kmax,tol);
%[w,f, gnorm] = stochasticNestrov(bsz,fun,gfun,w,kmax,tol);
figure;
plot(f,'Linewidth',2);
...
test = myquadratic2(Xtest,testlabel,w);
hits = find(test > 0);
misses = find(test < 0);
nhits = length(hits);
